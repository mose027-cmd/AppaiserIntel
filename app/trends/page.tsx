...
export default function TrendsPage() {
  ...
  useEffect(() => {
    async function loadSubmissions() {
      ...
      if (error) {
        console.error("Error loading assignment intelligence data:", error);
        setLoading(false);
        return;
      }
  ...
  const assignmentStats = useMemo(() => {
    ...
    submissions.forEach((item) => {
      const assignmentName = item.assignment_type?.trim();

      if (!assignmentName) return;

      if (!grouped[assignmentName]) {
        grouped[assignmentName] = [];
      }

      grouped[assignmentName].push(item);
    });
  ...
  const primaryAssignmentType = assignmentStats[0];
  ...
  const developingDatasetAssignments = assignmentStats
    ...
    return (
      <VerifiedAccess>
        <div className="rounded-3xl border border-slate-200 bg-white p-10 shadow-sm">
          <p className="text-slate-600">Loading assignment intelligence...</p>
        </div>
      </VerifiedAccess>
    );
  }
      ...
      <div className="space-y-8">
        <section className="rounded-3xl border border-slate-200 bg-white p-10 shadow-sm">
          ...
          <h1 className="mt-4 text-5xl font-bold tracking-tight text-slate-950">
            Assignment Benchmark Intelligence
          </h1>

          <p className="mt-4 max-w-4xl text-base leading-7 text-slate-600">
            Review assignment-type compensation patterns, turn-time performance,
        ...
        <section className="grid gap-6 md:grid-cols-4">
          <MetricCard
            title="Tracked Assignment Types"
            value={assignmentStats.length.toString()}
            subtitle="Assignment types with submitted activity"
          />

          <MetricCard
            title="Primary Assignment Type"
            value={primaryAssignmentType ? primaryAssignmentType.name : "N/A"}
            subtitle="Highest indexed assignment type by current benchmark data"
          />
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <AssignmentListCard
            ...
            subtitle="Assignment types currently trending above submitted market benchmarks"
            items={aboveBenchmarkAssignments}
            emptyMessage="No above-benchmark assignment cohort data available at the current contribution volume."
          />

          <AssignmentListCard
            title="Developing Dataset Cohort"
            subtitle="Assignment types with limited submitted data requiring additional contribution volume"
            items={developingDatasetAssignments}
            emptyMessage="No developing dataset assignment records available yet."
          />
        </section>
...
Assignment Benchmark Table
            ...
            <p className="mt-2 text-sm text-slate-500">
              Assignment-type benchmark positioning based on compensation, turn
              time, revision activity, net profit per day, and dataset
              confidence.
            </p>
          </div>
          ...
          <div className="mt-8 overflow-hidden rounded-2xl border border-slate-200">
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  <TableHeader>Assignment Type</TableHeader>
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {assignmentStats.length === 0 ? (
                  <tr>
                    ...
                    >
                      No assignment type data has been submitted yet.
                    </td>
                  </tr>
                ) : (
