...
const [amcSort, setAmcSort] = useState("count");
...
.sort((a, b) => {
  if (amcSort === "gross") return b.avgGrossFee - a.avgGrossFee;
  if (amcSort === "net") return b.avgNetFee - a.avgNetFee;
  if (amcSort === "turnTime") return a.avgTurnTime - b.avgTurnTime;
  return b.count - a.count;
});
  }, [filteredRecords, amcSort]);
...
<DataTableCard title={`Top AMCs by ${amcSort === "count" ? "Record Count" : amcSort === "gross" ? "Average Gross Fee" : amcSort === "net" ? "Average Net Fee" : "Fastest Turn Time"}`}>
                <div className="mb-4 flex justify-end">
                <select
                  className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm"
                  value={amcSort}
                  onChange={(e) => setAmcSort(e.target.value)}
                >
